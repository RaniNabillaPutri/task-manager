...
async function main() {
    try {
        ...
        const byNama = await db.collection('pengguna').findOne({ nama: 'Rani' });

        // Mencari satu dokumen dalam koleksi 'pengguna' berdasarkan ID objek tertentu.
        const byObjectID = await db.collection('pengguna').findOne({ _id: new ObjectId("6591802cae763a82e93871b2") });
